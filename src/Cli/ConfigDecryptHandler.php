...
final class ConfigDecryptHandler implements HandlerInterface
{
    public function run(RunnerInterface $runner): int|Response
    {
        ...
        if ($value === '') {
            $runner->io()->writeln('Укажите значение для расшифровки.', 'error');
        ...
        try {
            ...
            $plain = $encryptor->decrypt($value, $key);
        } catch (Throwable $e) {
            $runner->io()->writeln('Ошибка расшифровки: ' . $e->getMessage(), 'error');

            return Response::FAILURE;
        }

        $runner->io()->writeln($plain, 'success');
